import Footer from "../components/Footer";
import React, { useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import Logo from "../images/Logo.png";
import { FaArrowLeft } from "react-icons/fa";
import threeD from "../images/3d-face.jfif";

const ThreeD = () => {
  const [isOptionsVisible, setOptionsVisible] = useState(false);
  const [videoUrl, setVideoUrl] = useState("");
  const [output, setOutput] = useState("");
  const initials = "GM";
  const navigate = useNavigate();

  const toggleOptions = () => {
    setOptionsVisible((prevState) => !prevState);
  };

  const handleVideoInputChange = (e) => {
    setVideoUrl(e.target.value);
  };

  const handleBackButtonClick = () => {
    navigate(-1);
  };

  const handleSubmit = async () => {
    try {
      const response = await fetch("http://localhost:5000/process-video", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ videoUrl }),
      });

      if (!response.ok) {
        throw new Error("Network response was not ok");
      }

      const data = await response.json();
      setOutput(data.result); // Set the output result from the backend
    } catch (error) {
      console.error("Error:", error);
      setOutput("Error processing the video."); // Handle error scenario
    }
  };

  return (
    <div className="font-[Poppins] bg-gray-900 min-h-screen flex flex-col">
      <nav className="w-full p-4 shadow-lg bg-white text-black flex justify-between items-center">
        <img src={Logo} alt="LOGO" className="h-20 w-20" />
        <div
          className="w-12 h-12 bg-yellow-500 text-black flex items-center justify-center rounded-full text-lg font-semibold cursor-pointer hover:scale-105 transition-transform duration-300"
          onClick={toggleOptions}
        >
          {initials}
        </div>

        {isOptionsVisible && (
          <div className="absolute right-4 mt-12 space-y-2 font-bold bg-white p-4 rounded shadow-lg z-50">
            <Link
              to="/edit-details"
              className="block text-yellow-500 hover:underline text-lg transition duration-300"
              onClick={toggleOptions}
            >
              Edit Details
            </Link>
            <Link
              to="/login"
              className="block text-yellow-500 hover:underline text-lg transition duration-300"
              onClick={toggleOptions}
            >
              Logout
            </Link>
          </div>
        )}
      </nav>

      <div className="flex-grow flex flex-col items-center justify-center text-yellow-500 w-full p-6">
        <div
          className="flex items-center mb-4 cursor-pointer"
          onClick={handleBackButtonClick}
        >
          <FaArrowLeft
            size={40}
            className="hover:text-yellow-400 transition duration-300"
          />
          <h1 className="text-4xl ml-4">3D Face Reconstruction</h1>
        </div>

        <div className="bg-slate-800 p-6 rounded-lg shadow-lg w-full max-w-3xl overflow-hidden mb-6">
          <img
            src={threeD}
            alt="3D Face Reconstruction"
            className="w-full h-auto rounded-lg mb-4 shadow-md transition-transform duration-300"
          />
          <p className="text-lg text-center mb-4">
            This tool allows you to perform 3D face reconstruction from a video
            input. Follow the steps below to get started.
          </p>
          <h2 className="text-xl font-bold mt-4 text-center">Steps to Use:</h2>
          <ol className="list-decimal mx-4 text-left text-lg mb-4">
            <li>Upload your video input using the field below.</li>
            <li>Click on the 'Reconstruct' button to start processing.</li>
            <li>View the results displayed on the screen.</li>
          </ol>
        </div>

        <div className="mt-6 flex flex-col items-center w-full max-w-3xl">
          <input
            type="text"
            placeholder="Enter video URL or upload..."
            value={videoUrl}
            onChange={handleVideoInputChange}
            className="border border-yellow-500 p-3 rounded-lg w-full mb-2 focus:outline-none focus:ring-2 focus:ring-yellow-500 transition duration-300 shadow-lg"
          />
          <button
            onClick={handleSubmit}
            className="bg-yellow-500 text-black px-6 py-3 rounded-lg hover:bg-yellow-600 transition duration-300 shadow-md transform hover:scale-105 w-full"
          >
            Reconstruct
          </button>
        </div>

        {output && (
          <div className="mt-6 text-center bg-gray-800 p-4 rounded-lg shadow-lg w-full max-w-3xl">
            <h2 className="text-xl font-bold text-yellow-500">Output:</h2>
            <p className="text-lg text-white">{output}</p>
          </div>
        )}
      </div>

      <Footer />
    </div>
  );
};

export default ThreeD;
